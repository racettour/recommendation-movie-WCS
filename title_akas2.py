...
def table_cleaning():
    ...
    # ======================================================================================================
    # select no original titles in a new df
    print("finding fr title")
    df_no_org =df_title[df_title['isOriginalTitle'] == False]
    df_no_org = df_no_org[df_no_org['region']== 'FR']
    df_no_org = df_no_org.rename(columns={"title": "title_fr"})


    # get the region of the films thanks df_no_org
    titles_fr = df_no_org['title_fr']

    # past the region in df_org
    df_org =df_org.merge(titles_fr, how='left',  left_index=True, right_index=True, sort=True)

    df_org.drop(['ordering','isOriginalTitle'], axis=1, inplace=True)


    print("finding number of language")
    # creation of 2 df : the first one contains the number of language available by movie index
    df2 = df_title.groupby([df_title.index])['region'].count()
    df2 = df2.to_frame()
    df2 = df2.rename(columns={'region': 'language number'})  # old method

    df_org =df_org.merge(df2, how='left',  left_index=True, right_index=True, sort=True)

    df_org['region'] = df_org['region'].replace(np.nan, "no def")
    df_org['title_fr'].fillna(df_org['title'], inplace=True)
    df_org['language number'] = df_org['language number'].replace(np.nan, 1)
    df_org['language number'] = df_org['language number'].replace(0, 1)



    with open('df_akas.pkl', 'wb') as file:
        # A new file will be created
        pickle.dump(df_org, file)
